...
app.post('/createFiles', (req, res) => {
    ...
    const { numShards, numReplicas, protectedMode, portNumber, replicaOf, masterAuth, requirePass } = req.body;
    ...
    dockerCompose.services['app'] = {
        ...
        ports: ['80:80'],
        environment: ['REDIS_HOST:Redis-0-0',`REDIS_PORT:${portNumber}`, `REDIS_PASSWORD:${masterAuth}` ],
    ...
    for (let i = 0; i < numShards; i++) {
        for (let j = 0; j < numReplicas; j++) {
            ...
            // The first config file is for a master node and does not have the slaveof property
            if (j !== 1) redisConfigContent += `replicaof ${replicaOf} ${port}\n`;
            ...
            dockerCompose.services[`redis-${i}-${j}`] = {
                image: 'redis:latest',
                container_name: `redis-${i}-${j}`,
                volumes: [`./output/redis-${i}-${j}.conf:/usr/local/etc/redis/redis.conf`],
                command: `redis-server /usr/local/etc/redis/redis.conf`,
                ports: [`${port}:${portNumber}`]
            };

            // Increment port number as each Redis shard will run on a different port
            port++;
        }
    }
});
